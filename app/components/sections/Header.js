      ...
      <nav className="max-w-screen-xl mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between">
            ...
            <ul className="flex items-center space-x-8">
              {navigation.map((item, idx) => (
                <li key={idx}>
                  <Link
                    href={item.path}
                    className={`relative text-sm font-medium tracking-wide uppercase py-2 transition-colors duration-300 group ${
                      isScrolled ? 'text-foreground/80 hover:text-blue-600' : 'text-white/90 hover:text-white'
                    }`}
                  >
                    {item.title}
                    {/* Alt çizgi için de değişken kullandık */}
          ...
          <div className="flex items-center gap-x-4 md:hidden z-50">
            <ThemeToggle />
            <button className={`focus:outline-none p-2 transition-colors duration-300 ${state || isScrolled ? 'text-foreground' : 'text-white'}`} onClick={() => setState(!state)}>
              {state ? (

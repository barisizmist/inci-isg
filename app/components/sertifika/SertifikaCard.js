...
export default function SertifikaCard({ cert, onSelect, href, categoryLabel, showWhatsApp = true }) {
  const linkHref = href || `/sertifikalar/${cert.id}`;
  const categoryText = categoryLabel || cert.categoryName;
  const waHref = `https://wa.me/905466893488?text=${encodeURIComponent(`${cert.title} sertifikası hakkında bilgi almak istiyorum.`)}`;
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-auto">
          <div className="flex items-center justify-between gap-1 border-t pt-3" style={{ borderColor: 'var(--card-border)' }}>
            {FEATURES.map(f => (
              <span
                key={f.label}
                className="flex items-center gap-1 text-[9px] font-semibold whitespace-nowrap leading-none"
                style={{ color: 'var(--card-muted)' }}
              >
                <span aria-hidden="true" className="text-[10px] leading-none">{f.icon}</span>
                {f.label}
              </span>
            ))}
          </div>

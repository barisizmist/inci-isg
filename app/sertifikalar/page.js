...
// Data ve Bileşen Yolları
import { sertifikalarData, sertifikaKategorileri } from '@/data/sertifikalarData';
...
function SertifikalarContent() {
  const searchParams = useSearchParams();
  const urlCategory = searchParams.get('kategori');

  // URL'den kategori gelirse onu, gelmezse 'hepsi' değerini al
  const [selectedCategory, setSelectedCategory] = useState(urlCategory || 'hepsi');
  ...
  // URL parametresi dışarıdan (örn: Header'dan) değişirse state'i güncelle
  useEffect(() => {
    if (urlCategory) {
  ...
  const filteredCertificates = useMemo(() => {
    let result = sertifikalarData.filter(cert => {
...
// Next.js useSearchParams SSR Hatalarını Önlemek İçin Suspense Wrap Yapısı
export default function SertifikalarPage() {
  return (
    <Suspense fallback={<div className="min-h-screen pt-32 text-center text-sm text-muted-foreground">Yükleniyor...</div>}>
      <SertifikalarContent />
    </Suspense>
  );
}
